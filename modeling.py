...
class MAEEmbeddings(ViTEmbeddings):
    def __init__(self, config):
        super(MAEEmbeddings, self).__init__(config)
        delattr(self, "position_embeddings")
        self.register_buffer('position_embeddings', get_sinusoid_encoding_table(
            self.patch_embeddings.num_patches + 1, self.config.hidden_size))
...
class MAEEncoder(ViTPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config
        ...
        self.layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.pooler = MAEPooler(config) if add_pooling_layer else None
...
class MAEForMiM(ViTPreTrainedModel):
    def __init__(self, config, decoder_config):
        ...
        self.vit = MAEEncoder(config, add_pooling_layer=False)

        self.encoder_to_decoder = nn.Linear(config.hidden_size, decoder_config.hidden_size, bias=False)

        self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_config.hidden_size))
        self.register_buffer('decoder_position_embeddings', get_sinusoid_encoding_table(self.num_patches + 1,
                                                                                        decoder_config.hidden_size))
        self.decoder = MAEDecoder(decoder_config)
        self.decoder_norm = nn.LayerNorm(decoder_config.hidden_size, eps=config.layer_norm_eps)
        self.decoder_head = nn.Linear(decoder_config.hidden_size, self.patch_size ** 2 * 3)
    ...
    def forward(
        ...
        logits = self.decoder_head(self.decoder_norm(decoder_outputs[0][:, -pos_emd_mask.shape[1]:]))
...
class MAEForImageClassification(ViTForImageClassification):
    def __init__(self, config):
        super(MAEForImageClassification, self).__init__(config)
        self.num_labels = config.num_labels
